        ...
        for epochs in range(self.args.epochs):
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    optimizer.zero_grad()
                    # forward
                    outputs = model(text, vision, audio)['M']
                    # compute loss
            ...
            logger.info(
                ...
                f">> loss: {round(train_loss, 8)} {dict_to_str(train_results)}"
            )
            # validation
            ...
            if epochs == self.args.epochs:
                # save_path = self.args.model_save_dir + '/' + str(self.args.dataset_name) + '/' + self.args.model_name + '_' + str(self.args.dataset_name) + '_' + str(epochs) + '.pth'
                save_path = (self.args.model_save_dir / str(
                    self.args.dataset_name) / f"{self.args.model_name}_{self.args.dataset_name}_{epochs}.pth")
